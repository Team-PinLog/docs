...
Context **수정**이 기존 State를 재사용하지 않으므로, 수정으로 인해 기존 State를 `PENDING`으로 되돌리는 전이는 존재하지 않습니다. 예외로, **운영 재처리**(Embedding Profile 전환·프리셋 재분류)를 위해 Spring이 `COMPLETED` State를 `PENDING`으로 되돌리는 경로만 허용됩니다(§7.3). 이 경우에도 `PENDING` 쓰기 주체는 Spring이며(§6.4 무변경), FastAPI는 되살아난 `PENDING`을 신규 작업과 동일하게 선점·처리합니다.
...
- Spring이 그 값을 어디서 얻는지는 **검색 연동 Jira 작업에서 정합니다**. 소비자가 없는 상태에서 정하면 붙일 때 다시 뒤집힙니다.
...
**운영 재처리 경로.** Embedding Profile 전환이나 프리셋 재분류로 기존 `COMPLETED` Context를 다시 처리해야 할 때, Spring이 대상 State를 `PENDING`으로 되돌려 재처리 큐에 넣습니다(§6.3의 예외 전이). 이는 사용자 본문 수정과 무관한 **운영 배치**이며, 되살아난 `PENDING`은 FastAPI가 일반 신규 작업과 동일하게 선점·처리합니다. 재처리 범위·시점은 Spring의 운영 결정입니다.
...
```mermaid
flowchart LR
    Q[자연어 질의] --> RW[LLM 질의 재작성*]
    RW --> E[질의 전체 Embedding]
    E --> C[본인 Context exact cosine 검색]
    C --> G[Record ID 기준 집계]
    G --> K[Keyword 재정렬*]
    K --> S[Spring 문자열 검색·병합*]
    S --> V[Spring Core 재검증]
    V --> R[Record 목록 반환]
```

질의는 분해하지 않고 전체를 한 번 Embedding합니다. `*` 표시 단계는 기능 플래그 뒤의 선택 단계입니다(9.6). 플래그가 꺼져 있거나 단계가 실패하면 그 단계는 건너뛴 것과 같으며, 전부 건너뛴 흐름은 위에서 `*` 단계를 뺀 기본 흐름과 동일합니다.
...
`DISTINCT ON (record_id)`은 Record별 최고 유사도 Context 한 행만 남기며, 그 대표 Context의 `context_id`를 함께 반환합니다. Spring이 어느 Context가 매칭됐는지 알아야 Core에서 본문을 조회해 응답을 조립할 수 있기 때문입니다. FastAPI는 `context_id`만 반환하고 본문은 반환하지 않습니다.
...
한 Record의 여러 Context가 검색되더라도 해당 Record는 한 번만 반환합니다. Record 유사도는 그 Record에 속한 Context 유사도 중 최댓값을 기본값으로 사용하며, 그 **최고 유사도 Context의 `contextId`를 대표값으로 함께 반환**합니다. Spring은 이 id로 Core에서 본문을 조회해 `matchedContext`를 조립합니다(본문 자체는 FastAPI가 반환하지 않습니다).

MVP는 정확 cosine 검색을 사용합니다. 다음은 사용하지 않습니다.

- HNSW
- IVFFlat
- 검색어 LLM 분해 — 질의를 여러 검색어로 쪼개는 방식. 질의 하나를 하나로 다듬는 LLM 재작성(9.6)과 다르며, 재작성이 도입된 뒤에도 분해는 하지 않습니다
- 독립 Place 후보 검색
- 독립 Keyword 후보 검색 — Keyword로 후보를 새로 찾는 방식. 이미 뽑힌 후보의 순서만 바꾸는 Keyword 재정렬(9.6)과 다르며, 재정렬이 도입된 뒤에도 Keyword가 후보를 추가·제거하지 않습니다
- GraphRAG
...
기본 벡터 검색(9.2~9.5)에 세 가지 선택 신호를 더할 수 있습니다. 세 신호 모두 기본값이 꺼짐인 기능 플래그 뒤에 있으며, 전부 끄면 검색은 9.2의 기본 흐름과 동일하게 동작합니다. 플래그는 배포 통제 수단이 아니라 운영 중 문제 발생 시 기본 검색으로 되돌리는 안전장치입니다. 어느 단계가 실패해도 검색 응답은 실패하지 않습니다 — 해당 단계만 건너뜁니다.

**① LLM 질의 재작성 (FastAPI, `SEARCH_LLM_ENABLED`)**

Embedding 전에 LLM이 질의를 한 번 다듬습니다. 줄임말을 원말로 풀고(예: 부캠 → 부트캠프) 붙여 쓴 말을 띄어 씁니다. 다음을 계약으로 합니다.

- 앞뒤 공백을 정리한 질의가 6자 이하일 때만 재작성합니다(`SEARCH_REWRITE_MAX_CHARS`). 긴 질의는 LLM 호출 없이 원문으로 검색합니다.
- 재작성은 1:1 치환입니다. 질의를 여러 검색어로 쪼개지 않고, 결과 검색어 하나만 만듭니다.
- LLM 호출이 실패하거나 시간을 초과하면 원문으로 검색합니다. 이 경우의 동작은 재작성 도입 전과 같습니다.
- 같은 질의는 캐시로 같은 재작성 결과를 받습니다.
- 이후 Embedding과 유사도 판정은 재작성된 질의 기준입니다.

**② Keyword 재정렬 (FastAPI, `SEARCH_KEYWORD_RERANK_ENABLED`)**

기록 저장 시 판정해 둔 Keyword를 읽어, 유사도 판정을 통과한 후보의 순서를 조정합니다. 다음을 계약으로 합니다.

- 후보의 포함 여부를 바꾸지 않습니다. 재정렬 전후의 후보 Record id 집합은 같아야 하며 허용되는 차이는 순서뿐입니다. 재정렬 뒤 두 번째 후보 절단을 하지 않습니다.
- Keyword 판정이 완료된(`keyword_status = COMPLETED`) Context의 Keyword만 신호로 사용합니다.
- 응답의 `similarity`는 원래 cosine 값을 유지합니다. 재정렬 점수는 노출하지 않습니다.
- 조회·계산이 실패하거나 신호가 없으면 유사도 순서를 그대로 반환합니다.

**③ 본문 문자열 검색과 병합 (Spring, `pinlog.search.lexical.enabled`)**

본문(`core.context.body`)에 질의 문자열이 그대로 있는 기록을 찾아 벡터 결과와 합칩니다. 본문은 Spring 소유이므로 이 검색과 병합은 Spring이 수행하며, FastAPI는 여전히 본문에 접근하지 않습니다. 다음을 계약으로 합니다.

- 앞뒤 공백을 정리한 질의에 공백이 없고 5자 이하인 단어형 질의에서만 켭니다. 매치는 부분일치입니다.
- 검색 범위는 요청자 소유의 삭제되지 않은 본문뿐입니다.
- 벡터 결과와 문자열 매치의 합집합을 RRF(k=60) 순위 결합으로 재정렬하고, 요청 개수(`size`) 절단은 재정렬 뒤 한 번만 합니다.
- 문자열 매치로만 들어온 항목의 `similarity`는 0.0입니다(cosine 값이 존재하지 않으며, 실제 cosine은 0.0이 나오지 않으므로 구분 표지가 됩니다). 벡터 후보의 `similarity`는 원래 cosine 값을 유지합니다.
- 문자열 후보도 9.5의 Spring 최종 검증을 동일하게 거칩니다.
- 문자열 검색이 실패하면 벡터 결과만 반환합니다.

### 9.7 관련도 재판정 (검색 4번째 신호)

9.2의 세 신호(벡터 검색·문자열 검색·키워드 재정렬)와 9.5의 Spring 재검증까지 끝난 최종
후보에 4번째 신호로 LLM 관련도 재판정을 얹을 수 있습니다. 계약은 13.3을 따릅니다. 기본
꺼짐이며, 켜지 않은 배포에서는 이 절이 없는 것과 같습니다.
...
- `recordId`
- `contextId` — 그 Record에서 매칭된 대표(최고 유사도) Context의 id. Spring이 Core에서 본문을 조회해 `matchedContext`를 조립하는 데 사용합니다. FastAPI는 id만 반환하고 본문은 반환하지 않습니다.
- `similarity`

`userId`는 필수입니다. FastAPI는 이 값을 검색 범위 필터로 사용하며, 인증 자체를 판단하지 않습니다.

### 13.3 검색 결과 관련도 재판정 (검색 4번째 신호)

```text
POST /internal/v1/search/judge
```

목적: 개인 자연어 검색의 4번째 신호. Spring이 9.2의 세 신호(벡터 검색·문자열 검색·키워드
재정렬)를 모두 병합한 **최종 후보**를 LLM에게 질의와 함께 보여 관련도 4단계로 재판정받고,
무관한 결과를 제거·재정렬한다. 문장형 질의에 포함된 고유명사처럼 세 신호 모두의 사각지대를
메우는 보조 신호이며, 세 신호를 대체하지 않는다.

주요 요청값:

- `query`
- `candidates` — `contextId`·`placeName`·`body`의 배열. **body를 포함한다.**

주요 응답값:

- `results` — `contextId`·`relevance`(`VERY_RELEVANT`·`RELEVANT`·`WEAKLY_RELEVANT`·
  `NOT_RELEVANT` 중 하나)의 배열

**이 요청은 본문을 싣습니다 — 9.3·9.4·13.2의 "FastAPI는 본문을 반환하지 않는다"는 조항과
방향이 다릅니다.** 그 조항들은 **ai→back 응답**에 본문이 없다는 뜻이며, 이 엔드포인트가 쓰는
**back→ai 요청** 방향을 막지 않습니다. ai(FastAPI)는 Context 본문을 저장하지 않으므로(12.3·
12.4 어디에도 본문 컬럼이 없습니다), 판정에 본문이 필요하면 유일한 소유자인 back이 요청에
실어 보내는 수밖에 없습니다. `POST /internal/v1/context/process`의 `text`(13.1)가 이미 같은
방향(back→ai, 본문 포함)의 선례입니다.

이 신호는 기본적으로 꺼져 있습니다. 켜는 것은 back의 기능 플래그(`pinlog.search.relevance-judge.enabled`)
결정이며, 꺼진 상태의 검색 응답은 이 신호가 없는 것과 완전히 같습니다. 호출이 실패해도
검색 자체를 실패시키지 않습니다 — Spring은 판정 직전의 후보 순서로 되돌립니다. 신뢰도 게이트
(`pinlog.search.gate.enabled`)도 켜진 배포에서는 이 순서가 **게이트 적용 후** 순서입니다. 따라서
게이트에서 제외된 후보는 LLM에 전달되지 않으며, LLM 실패가 해당 후보를 복구하지 않습니다.
...
- Redis Feed Cache
- 검색 신호 확장 3종(LLM 질의 재작성·Keyword 재정렬·본문 문자열 검색, 기본 꺼짐 플래그·9.6)
...
- GraphRAG
- 자유 생성 공개 Keyword
- 검색어 LLM 분해(질의를 여러 검색어로 쪼개는 방식 — 1:1 재작성은 9.6으로 포함)
- 학습형 Feed Ranking
- Multi-Armed Bandit
